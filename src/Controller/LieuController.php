...
class LieuController extends AbstractController
{
		...
		public function new(Request $request, LieuRepository $lieuRepository): Response
		{
				...
				if ($form->isSubmitted() && $form->isValid())
				{
						...
						{
								$this->addFlash('warning', 'Le lieu ' . $lieu->getNom() . ' n\'a pu être créé!');
						}

						return $this->redirectToRoute('sortie_new', ['lieu' => $lieu, 'form' => $form,]);
				}

				return $this->renderForm('lieu/new.html.twig', ['lieu' => $lieu, 'form' => $form]);
		}
		...
		public function edit(Request $request, Lieu $lieu, LieuRepository $lieuRepository): Response
		{
				...
				if ($form->isSubmitted() && $form->isValid())
				{
						...
						{
								$this->addFlash('warning', 'Le lieu ' . $lieu->getNom() . ' n\'a pu être modifié!');
						}

						return $this->redirectToRoute('lieu_list', [], Response::HTTP_SEE_OTHER);
				}

				return $this->renderForm('lieu/edit.html.twig', ['lieu' => $lieu, 'form' => $form,]);
		}
		...
		public function delete(Request $request, Lieu $lieu, LieuRepository $lieuRepository): Response
		{
				if ($this->isCsrfTokenValid('delete' . $lieu->getId(), $request->request->get('_token')))
				{
						...
						{
								$this->addFlash('warning', 'Vous ne pouvez pas supprimer le lieu ' . $lieu->getNom() . ' car il est référencé dans la base de données!!!');
						}
				}
